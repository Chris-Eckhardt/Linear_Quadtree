...
void draw_character (VGA_WINDOW * wnd, int x, int y, int bg_color, int fg_color, char c, int canvas);

void draw_string (VGA_WINDOW * wnd, int x, int y, int bg_color, int fg_color, const char * str, int canvas);
...
void create_window ( PARAM_VGA_CREATE_WINDOW * params)
{
    ...
    window->frame.title = params->title;
...
void draw_string (VGA_WINDOW * wnd, int x, int y, int bg_color, int fg_color, const char * str, int canvas) {
	...
	while (*(str) != '\0') {
		draw_character(wnd, x, y, bg_color, fg_color, *(str), canvas);
		str++;
		x+=8;
	}
}
		...
		for (n = 0; n < FONT_SIZE; n++) {
			...
			if ((b>>n&1)) {
                if(canvas)
                    ...
                    set_pixel(wnd, fb.x+x+FONT_SIZE-n-1, fb.y+y+i, fg_color);
			} else {
                if(canvas)
				    set_canvas_pixel(wnd, x+FONT_SIZE-n-1, y+i, bg_color);
                else
                    set_pixel(wnd, fb.x+x+FONT_SIZE-n-1, fb.y+y+i, bg_color);
			}
		}
	}
}
...
void vga_draw_frame(VGA_WINDOW * window)
{
    ...
    draw_string(window, (fb.width/2)-((k_strlen(window->frame.title)/2)*8), 2, WHITE, BLACK, window->frame.title, 0);
}
